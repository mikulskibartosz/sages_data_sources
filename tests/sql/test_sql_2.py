...
class TestSql:
    ...
    def test_load_passengers_without_tickets(self, load_titanic_data):
        ...
        """
        df = run_query('SELECT Name FROM passengers WHERE PassengerId NOT IN (SELECT PassengerId FROM tickets)')
    ...
    def test_load_passengers_and_their_cabin_numbers_keep_passengers_without_tickets(self, load_titanic_data):
        ...
        """
        df = run_query("SELECT Name, Cabin FROM passengers left outer join tickets on passengers.PassengerId = tickets.PassengerId")
    ...
    def test_load_passengers_and_their_ticket_numbers_remove_passengers_without_tickets(self, load_titanic_data):
        ...
        """
        df = run_query("SELECT Name, Ticket FROM passengers inner join tickets on passengers.PassengerId = tickets.PassengerId")
